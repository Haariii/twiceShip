...
namespace aw
{
	...
	void Game::initMusic()
	{
		if (m_gameType == GameType::OFFICIAL_ARCADE)
		{
			...
			{
				if (m_openMusic != 0)
					m_music.openFromFile("data/music/twice like ooh ahh.ogg");
			...
			{
				if (m_openMusic != 1)
					m_music.openFromFile("data/music/twice knock knock.ogg");
			...
			{
				if (m_openMusic != 2)
					m_music.openFromFile("data/music/twice likey.ogg");
...
void initGui(aw::GuiController &gui)
{
	...
	gui.addButton(0, "msg3", sf::Vector2f(175, 180), "Space: Steer up the ship");
	gui.getElement(0, 3)->setCharacterSize(35);
	gui.getElement(0, 3)->setSelectable(false);
