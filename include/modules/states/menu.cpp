...
namespace aw
{
		...
		m_messageBus(messageBus)
	{
		...
		m_texLogo.loadFromFile("data/images/twice.png");
		m_logo.setTexture(m_texLogo);
		m_logo.setScale(0.6f, 0.6f);
		m_logo.setPosition(225.f, -10.f);

		resetView();

		m_music.openFromFile("data/music/twice what is love.ogg");
	}
	...
	void Menu::receiveMessage(const Message &msg)
	{
		if (msg.ID == aw::hash("sound settings"))
		{
			...
			//Update the gui to display the right values
			...
			if (m_levelInformation.name != name)
			{
				...
				else
				{
					...
					if (name.substr(0, 8) == "Tutorial")
					{
						...
						m_levelInformation.author = "Harri";
...
void initMainLayer(aw::GuiController &gui)
{
	...
	gui.addButton(0, "arcade", sf::Vector2f(275, 205), "Play now");
	gui.addButton(0, "exit", sf::Vector2f(275, 275), "Close");
}
...
void initCreditsLayer(aw::GuiController &gui)
{
	...
	gui.addLabel(6, "credit", sf::Vector2f(50, 145), "Developed by Harri Hoang Tran.");
}
